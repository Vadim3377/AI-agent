...
class DomainProfiler:
    ...
    The profiler converts that into capabilities, tools, evaluation metrics,
    and reasoning that the StemShell uses to generate a blueprint.

    Classification metadata (method and LLM reasoning) is preserved in the
    returned DomainProfile so callers can surface it without re-classifying.
    """
    ...
    def _debugging_profile(self, classification: ClassificationResult) -> DomainProfile:
        return DomainProfile(
            ...
            llm_reasoning=classification.llm_reasoning or "",
        )
